...
function IdeaLink({ item }) {
  ...
  const Post = async () => {
    try {
      const response = await instance.patch(`/post/${item.id}`, newPost, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("access-token")}`,
        },
      });
  ...
  const deletePost = async () => {
    try {
      const response = await instance.delete(`/post/${item.id}`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("access-token")}`,
        },
      });
  ...
  return (
    ...
    `}
      ...
      >
        {!mode ?
          <>
            <BsThreeDotsVertical size={36} onClick={() => setDropdown(prev => !prev)} css={css`
          cursor: pointer;
          color: #898989;
          position: absolute;
          right: 100px;
        `} />
          ...
          `}>
              ...
              cursor: pointer;
            `} onClick={() => setMode(true)}>수정하기</span>
                <span css={css`
              height: 36px;
              width: 100px;
              line-height: 36px;
        ...
        `}>
            ...
            text-align: center;
          `}>
                <img src={`${newPost.imageUrl ? newPost.imageUrl : item.imageUrl}`} alt='프로젝트 로고' css={css`
              width: 250px;
              height: 250px;
              border-radius: 20px;
            `} />
